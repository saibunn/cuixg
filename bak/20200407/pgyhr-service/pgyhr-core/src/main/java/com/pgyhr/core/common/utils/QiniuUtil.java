...
public class QiniuUtil {
     ...
     * 生成上传凭证，然后准备上传
     */
    @Value("${pgyhr.qiniu.accessKey}")
    private String accessKey;

    @Value("${pgyhr.qiniu.secretKey}")
    private String secretKey;

    @Value("${pgyhr.qiniu.bucket}")
    private String bucket;

    @Value("${pgyhr.qiniu.domain}")
    private String domain;

    @Value("${pgyhr.qiniu.zone}")
    ...
    public String qiniuUpload(String filePath, String key) {
        ...
        try {
            ...
            throw new PgyhrException("上传文件出错，请检查七牛云配置，" + r.toString());
        }
    }
